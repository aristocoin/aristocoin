...
#if defined(NDEBUG)
# error "AristoCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x4053f27502d05555eab8855c2ee464798c2a4a541a242ea75a52add46471d8ba");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // AristoCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "AristoCoin Signed Message:\n";
...
{
    // AristoCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    //nSubsidy >>= (nHeight / 840000); // AristoCoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 6 * 60 * 60;; // AristoCoin: 6 hour
static const int64 nTargetSpacing = 2 * 60; // AristoCoin: 2 Minutes.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // AristoCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // AristoCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x4053f27502d05555eab8855c2ee464798c2a4a541a242ea75a52add46471d8ba");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is AristoCoin 2 August 2017";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;	
        block.nTime    = 1501682254;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2362152;
        ...
        assert(block.hashMerkleRoot == uint256("0xd9834348f5a42b337075df25f1d436104f3f2b398fe6cf023ca9bb12ee30f59e"));
        ...
        if (true && block.GetHash() != hashGenesisBlock)
{
    ...
    uint256 thash;
    char scratchpad[SCRYPT_SCRARISTOHPAD_SIZE];
...
unsigned char pchMessageStart[4] = { 0xc3, 0xd2, 0xd1, 0xbd }; // AristoCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AristoCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AristoCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AristoCoinMiner(CWallet *pwallet)
{
    printf("AristoCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("aristocoin-miner");
    ...
    try { loop {
        ...
        printf("Running AristoCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AristoCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AristoCoinMiner, pwallet));
}
